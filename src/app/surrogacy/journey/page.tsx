...
function JourneyInner() {
  ...
  React.useEffect(() => {
    ...
    // 没有 caseId 参数时，自动获取 surrogacyId 并请求第一个 caseId
    const surrogacyId = typeof window !== 'undefined' ? localStorage.getItem('surrogacyId') : null;
    if (!surrogacyId) return;
    fetch(`/api/cases-by-surrogacy?surrogacyId=${surrogacyId}`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleViewClick = (stageNumber: number, itemTitle: string) => {
    if (!caseId) return;
    router.push(`/surrogacy/files?caseId=${caseId}&stage=${stageNumber}&title=${encodeURIComponent(itemTitle)}`);
  };
