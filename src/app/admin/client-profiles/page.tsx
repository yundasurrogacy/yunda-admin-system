              ...
              <div key={client.id} className="bg-white rounded-lg border border-sage-200 p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h3 className="text-sage-800 font-medium">{basic.firstName} {basic.lastName}</h3>
                      <span className="text-sm text-sage-500">{client.id}</span>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-2">
                    {/* 状态字段可根据业务补充 */}
                    <span className={`px-2 py-1 rounded-full text-xs bg-sage-100 text-sage-800`}>
                      {/* 可根据业务映射 */}Active
                    </span>
                    <span className={`px-2 py-1 rounded-full text-xs ${getServiceColor(service)}`}>
                      {service}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-2 mb-4">
                  ...
                  <div className="flex items-center gap-2 text-sm text-sage-600">
                    ...
                    <span>{contact.cell_phone_country_code ? `+${contact.cell_phone_country_code} ` : ""}{contact.cell_phone}</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm text-sage-600">
                    <MapPin className="w-4 h-4" />
                    <span>{family.city}, {family.state_or_province}, {family.country}</span>
                  </div>
                </div>

                <div className="flex items-center justify-between mt-4 pt-4 border-t border-sage-100">
                  <span className="text-sm text-sage-500">
                    {text[language].lastUpdate}: {client.updated_at?.slice(0, 10) || "-"}
                  </span>
                  <Button
                    variant="link"
