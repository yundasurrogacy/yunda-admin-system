...
export default function ClientManagerPage() {
  ...
  const handleResetPassword = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/managers-reset-password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: resetManagerId, password: resetPassword })
      })
      const result = await res.json()
      if (res.ok && result?.update_managers?.affected_rows > 0) {
        ...
        {loading ? (
          ...
          <div className="flex items-center justify-center" style={{ minHeight: '400px' }}>
            <div className="text-center">
              ...
              <p className="text-xl text-sage-600 font-medium mb-2">{t('noAdmin')}</p>
              <p className="text-sm text-sage-400 mb-6">{t('noAdminDesc', { defaultValue: '暂无管理员记录' })}</p>
            </div>
          </div>
        ) : (
          <>
              ...
              {pagedManagers.map((m) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-sage-500 text-sm">
                    ...
                    <CustomButton
                      className="text-sage-600 px-2 py-1 border border-sage-300 cursor-pointer bg-white text-sm"
            ...
            <div className="bg-white rounded-2xl shadow-2xl border border-sage-200 w-full max-w-md p-8 animate-fade-in">
              ...
              <div className="flex justify-end gap-3">
                <CustomButton 
                  ...
                  className="px-6 py-2 rounded-lg border border-sage-300 bg-white hover:bg-sage-50 cursor-pointer"
                >
                  {t('cancel')}
                </CustomButton>
                <CustomButton 
                  ...
                  className="px-6 py-2 bg-sage-600 text-white rounded-lg shadow hover:bg-sage-700 transition cursor-pointer disabled:bg-gray-400 disabled:cursor-not-allowed"
                >
