...
import { exportDetailToExcel, exportDetailToPdf, exportSurrogateDetailFixedToExcel, type DetailExportRow } from '@/lib/exports/applications'
import { buildSurrogateDetailFixedRow, parseSurrogateApplicationData, type SurrogateParsedData } from '@/lib/exports/surrogate-fixed-rows'
...
export default function SurrogateApplicationDetailPage() {
  ...
  // 返回列表的页码：优先用 sessionStorage（列表点进详情时写入），避免 fromPage 在 Next 中未正确传递
  const getReturnPage = useCallback(() => {
    if (typeof window === 'undefined') return fromPage
    const saved = sessionStorage.getItem('surrogates-list-return-page')
    if (saved) return saved
    return fromPage
  }, [fromPage])
  const listUrlWithPage = `/admin/surrogates-applications?page=${getReturnPage()}`

  // 使用 useCallback 缓存事件处理函数（返回时带上原列表页码）
  const handleBack = useCallback(() => {
    const page = getReturnPage()
    router.push(`/admin/surrogates-applications?page=${page}`)
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      router.push(`/admin/surrogates-applications?page=${getReturnPage()}`)
    }
    catch (error) {
  ...
  // 导出行：必须在所有条件 return 之前调用，保证 Hooks 顺序一致；依赖语言以便切换后导出与页面一致
  ...
  }, [application, parsedData, t, i18n.language])

  // 固定列导出：一行一个孕妈，分娩记录 10 列，附加信息列预留
  const fixedExportData = useMemo(() => {
    if (!application || !parsedData) return null
    return buildSurrogateDetailFixedRow(application, parsedData as SurrogateParsedData, t)
  }, [application, parsedData, t, i18n.language])
  ...
  const handleExportDetail = useCallback((format: 'excel' | 'pdf') => {
    ...
    if (format === 'excel') {
      if (fixedExportData) {
        exportSurrogateDetailFixedToExcel(fixedExportData.headers, [fixedExportData.row], `${baseName}.xlsx`)
      } else {
        if (!detailExportRows.length) {
          window.alert(t('noRecords', { defaultValue: 'No records' }))
          return
        }
        exportDetailToExcel(detailExportRows, `${baseName}.xlsx`)
      }
      return
    }
  ...
  }, [application?.id, detailExportRows, fixedExportData, t])
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <CustomButton
            onClick={() => router.push(listUrlWithPage)}
