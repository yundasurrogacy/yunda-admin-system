...
function JourneyInner() {
  ...
  const handleViewClick = useCallback((stageNumber: number, itemTitle: string) => {
    ...
    router.push(`/client-manager/files?caseId=${caseId}&stage=${stageNumber}&title=${encodeURIComponent(itemTitle)}${journeyId ? `&journeyId=${journeyId}` : ''}&about_role=intended_parent`);
